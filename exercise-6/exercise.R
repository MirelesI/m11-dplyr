...
average.arrival.delay <- flights %>% 
  group_by(dest) %>% 
  summarise(avg.delay = mean(arr_delay, na.rm = TRUE)) %>% mutate(faa = dest) %>% 
  left_join(airports, by = 'faa') %>% arrange(-avg.delay)
...
average.delay.airline <- flights %>% 
  group_by(carrier) %>% summarise(avg.delay = mean(arr_delay, na.rm = TRUE)) %>% 
